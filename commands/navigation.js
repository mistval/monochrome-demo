...
module.exports = {
  ...
  action(bot, msg, suffix, monochrome) {
    const randomNumberChapter = new NavigationChapter(new RandomNumberDataSource());
    ...
    const latinChapter = NavigationChapter.fromContent([latinPar1, latinPar2, latinPar3]);
    ...
    const galaxyPictureChapter = NavigationChapter.fromContent([galaxyPictureContent1, galaxyPictureContent2]);

    const chapterForReaction = {
      '🇮🇹': latinChapter,
      '⭐': galaxyPictureChapter,
      '🐺': randomNumberChapter,
    };

    const navigation = new Navigation(msg.author.id, true, '🇮🇹', chapterForReaction);
    const navigationManager = monochrome.getNavigationManager();
    return navigationManager.show(navigation, 10000000, msg.channel, msg);
  },
};
